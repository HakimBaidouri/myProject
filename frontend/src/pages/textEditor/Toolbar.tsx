            ...
            <DropdownMenuContent className="p-1">
                {lineHeights.map((lineHeight) => {
                    ...
                    return (
                        ...
                        onClick={() => editor?.chain().focus().setLineHeight(lineHeight.value).run()}
                        className={cn(
                            "flex items-center gap-x-2 px-2 py-1 rounded-sm hover:bg-neutral-200/80",
                            editor?.getAttributes('"paragraph').lineHeight === lineHeight.value && "bg-neutral-200/80"
                        )}
                        >
            ...
            <DropdownMenuContent className="p-1">
                {lists.map((list) => {
                    ...
                    return (
                        ...
                        className={cn(
                            "flex items-center gap-x-2 px-2 py-1 rounded-sm hover:bg-neutral-200/80",
                            list.isActive() && "bg-neutral-200/80"
                        )}
                        >
...
const AlignButton = () => {
    ...
    return(
        <DropdownMenu>
            ...
            <DropdownMenuContent className="p-1">
                {alignments.map((alignement) => {
                    const Icon = alignement.icon;

                    return (
                        <button
                        key={alignement.value}
                        onClick={() => editor?.chain().focus().setTextAlign(alignement.value).run()}
                        className={cn(
                            "flex items-center gap-x-2 px-2 py-1 rounded-sm hover:bg-neutral-200/80",
                            editor?.isActive({ textAlign: alignement.value }) && "bg-neutral-200/80"
                        )}
                        >
                        <Icon className="size-4" />
                        <span className="text-sm">{alignement.label}</span>
                        </button>
                    );
                })}
            </DropdownMenuContent>
