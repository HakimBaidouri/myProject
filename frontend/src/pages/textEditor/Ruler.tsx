  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    if ((isDraggingLeft || isDraggingRight) && rulerRef.current) {
      ...
      if (container) {
        ...
        let newLeft = leftMargin;
        let newRight = rightMargin;

        if (isDraggingLeft) {
          const maxLeftPosition = 816 - rightMargin - 100;
          newLeft = Math.min(rawPosition, maxLeftPosition);
        ...
        if (isDraggingRight) {
          ...
          newRight = Math.min(newRightPosition, maxRightPosition);
          setRightMargin(newRight);
        }

        onMarginsChange?.(newLeft, newRight);
      }
    }
  };
