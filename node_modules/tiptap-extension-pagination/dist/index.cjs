...
const setSelectionAtPos = (tr, anchor, head) => {
  const selection = state.TextSelection.create(tr.doc, anchor, anchor);
...
const moveToPreviousTextBlock = (tr, $pos) => {
  ...
  const selection = state.Selection.near(prevResPos, searchDirection);
  return selection;
};
/**
...
const moveToThisTextBlock = (tr, $pos, bias = 1, offsetInNode = 0) => {
  ...
  const selection = state.Selection.near($offsetPos, bias);
  return selection;
};
/**
...
const moveToNextTextBlock = (tr, $pos) => {
  ...
  const searchDirection = 1;
  const selection = state.Selection.near(nextResPos, searchDirection);
  return selection;
};
/**
...
const getNearestTextSelection = ($pos, bias = 1) => {
  return state.TextSelection.near($pos, bias);
};
/**
...
const moveToNearestValidCursorPosition = $pos => {
  const selection = state.Selection.findFrom($pos, 1, true) || state.Selection.findFrom($pos, -1, true);
  return selection;
};
...
const KeymapPlugin = keymap.keymap({
  ...
  Enter: (state, dispatch) => {
    ...
    if (core.isNodeEmpty(paragraphNode)) {
  ...
  Backspace: (state, dispatch) => {
    ...
    if (isPosAtEndOfBody(doc, $pos)) {
      ...
      if (!core.isNodeEmpty(previousParagraphNode) || !core.isNodeEmpty(paragraphNode)) {
        deleteNode(tr, paragraphPos, paragraphNode);
      }
  ...
  Delete: (state, dispatch) => {
    ...
    const thisNodeEmpty = core.isNodeEmpty(paragraphNode);
    const nextNodeEmpty = core.isNodeEmpty(nextParagraphNode);
    if (!nextNodeEmpty) {
...
const buildNewDocument = (editor, options, contentNodes, nodeHeights) => {
  ...
  const constructPageRegions = currentPageContent => {
    ...
    const pageRegions = [currentPageHeader, pageBody, pageFooter];
...
const PaginationPlugin = ({
  ...
  return new state.Plugin({
    key: new state.PluginKey("pagination"),
    view() {
 ...
 */
const PaginationExtension = core.Extension.create({
...
const constructChildOnlyClipboardSerialiser = (schema, isNode) => {
  ...
  const clipboardSerialiser = Object.create(model.DOMSerializer.fromSchema(schema));
  // Override serializeFragment
  clipboardSerialiser.serializeFragment = (fragment, options = {}, target = document.createDocumentFragment()) => {
    const serializer = model.DOMSerializer.fromSchema(schema);
...
const PageNode = core.Node.create({
  name: PAGE_NODE_NAME,
  group: "block",
  content: `block{1,3}`,
  ...
  renderHTML({
    ...
    return [baseElement$2, core.mergeAttributes(HTMLAttributes, {
...
const HeaderFooterNode = core.Node.create({
  ...
  renderHTML({
    ...
    return [baseElement$1, core.mergeAttributes(HTMLAttributes, {
...
const BodyNode = core.Node.create({
  name: BODY_NODE_NAME,
  group: "block",
  content: "block*",
  defining: true,
  ...
  renderHTML({
    ...
    return [baseElement, core.mergeAttributes(HTMLAttributes, {
      [bodyAttribute]: true,
...
exports.DARK_PAPER_COLOUR = DARK_PAPER_COLOUR;
exports.DEFAULT_PAGE_AMENDMENT_CONFIG = DEFAULT_PAGE_AMENDMENT_CONFIG;
exports.DEFAULT_PAGE_BORDER_CONFIG = DEFAULT_PAGE_BORDER_CONFIG;
exports.DEFAULT_PAGE_MARGIN_CONFIG = DEFAULT_PAGE_MARGIN_CONFIG;
exports.DEFAULT_PAGE_MARGIN_NAME = DEFAULT_PAGE_MARGIN_NAME;
exports.DEFAULT_PAPER_COLOUR = DEFAULT_PAPER_COLOUR;
exports.DEFAULT_PAPER_ORIENTATION = DEFAULT_PAPER_ORIENTATION;
...
exports.doesDocHavePageNodes = doesDocHavePageNodes;
exports.getBodyNodeMargins = getBodyNodeMargins;
exports.getPageNodePageBorders = getPageNodePageBorders;
exports.getPageNodePaperColour = getPageNodePaperColour;
exports.getPageNodePaperOrientation = getPageNodePaperOrientation;
exports.getPageNodePaperSize = getPageNodePaperSize;
exports.getPageNumPageBorders = getPageNumPageBorders;
exports.getPageNumPageMargins = getPageNumPageMargins;
exports.getPageNumPaperColour = getPageNumPaperColour;
exports.getPageNumPaperOrientation = getPageNumPaperOrientation;
exports.getPageNumPaperSize = getPageNumPaperSize;
exports.getPageNumber = getPageNumber;
exports.getThisPageNodePosition = getThisPageNodePosition;
exports.isPageNode = isPageNode;
exports.pageSides = pageSides;
exports.paperDimensions = paperDimensions;
exports.paperOrientationsSelect = paperOrientationsSelect;
